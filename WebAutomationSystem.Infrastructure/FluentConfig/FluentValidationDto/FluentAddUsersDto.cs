...
{
    public class FluentAddUsersDto : AbstractValidator<AddUsersDto>
    {
        public FluentAddUsersDto()
        {
            #region FirstName
            ...
            RuleFor(user => user.UserName).MinimumLength(4)
                .WithMessage("نام کاربری باید حداقل 4 کاراکتر باشد.");

            RuleFor(user => user.UserName).MaximumLength(50)
                .WithMessage("نام کاربری باید حداکثر 50 کاراکتر باشد.");
            ...
            RuleFor(user => user.Password).MinimumLength(4)
                .WithMessage("رمز عبور باید حداقل 4 کاراکتر باشد.");

            RuleFor(user => user.Password).MaximumLength(25)
                .WithMessage("رمز عبور باید حداکثر 25 کاراکتر باشد.");
            ...
            RuleFor(user => user.ConfirmPassword).NotNull()
                .WithMessage("تکرار رمز عبور را وارد کنید.");

            RuleFor(user => user.ConfirmPassword).Equal(user => user.Password)
                .WithMessage("تکرار رمز عبور با رمز عبور همخوانی ندارد.");
